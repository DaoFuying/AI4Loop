...
if __name__=='__main__':
    parser = argparse.ArgumentParser(description="Generate random pairs of gene regions")
    parser.add_argument("name", help="The prefix of the dataset. For example k562_polr2a.")
    parser.add_argument("input_file", help="The gene/TF file in BED format")
    parser.add_argument("type", choices=['gene', 'tf'],
                        help="Can be gene or tf, indicating whether this is for gene or TF peaks pairs")
    parser.add_argument("datadir", help="The directory where the input and output are in.")
    args = parser.parse_args()

    anchor_file = "{}/{}_merged_anchors.both_gene.bed".format(args.datadir, args.name)
    inter_file = "{}/{}.clustered_interactions.both_gene.bedpe".format(args.datadir, args.name)
